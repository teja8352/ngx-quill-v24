...
import { defaultModules, QuillModules, CustomOption, CustomModule } from 'projects/ngx-quill-v24/config/src/public_api'
...
export abstract class QuillEditorBase implements AfterViewInit, ControlValueAccessor, OnChanges, OnInit, OnDestroy, Validator {
  ...
  /*
  https://github.com/KillerCodeMonkey/ngx-quill-v24/issues/1257 - fix null value set
  ...
  */
  ...
  ngAfterViewInit() {
    ...
    this.quillSubscription = this.service.getQuill().pipe(
      ...
      if (this.content) {
        ...
        const historyModule: any = this.quillEditor.getModule('history');
        historyModule.clear();
  ...
  validate() {
    ...
    const onlyEmptyOperation = deltaOperations && deltaOperations.length === 1 && ['\n', ''].includes(deltaOperations[0].insert as any)
